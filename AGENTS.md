...
## Project Snapshot
- Expo Router + React Native app (`main: expo-router/entry`) with TypeScript strict mode; current route tree is minimal (`app/_layout.tsx`, `app/index.tsx`).
...
- No service layer/state manager exists yet; logic is currently expected to live inside route components.
...
- Camera capability is already installed (`expo-camera` in `package.json`) and partially imported in `app/index.tsx`; complete permission + capture flow there or remove unused imports to keep lint clean.
